<script lang="ts">
	...
	import type { Note, NoteDelta, NoteEvent } from '$lib/protobuf/notes';
	...
	function handleRealtimeEvent(event: NoteEvent): void {
		...
		if (event.event.kind === 'created') {
			upsertNote(event.event.note);
			return;
		}

		if (event.event.kind === 'updated') {
			const shouldSyncEditor = !hasUnsavedChanges && selectedNoteId === event.event.delta.id;
			const updated = applyDelta(event.event.delta);
		...
		removeNote(event.event.deleted.id);
	}
	...
	function applyDelta(delta: NoteDelta): Note | null {
		...
		const updated: Note = {
			...
			updated_at_unix_ms: delta.updated_at_unix_ms,
	...
	function sortNotes(items: Note[]): Note[] {
		return [...items].sort((a, b) => {
			if (a.updated_at_unix_ms !== b.updated_at_unix_ms) {
					...
					{#each notes as note (note.id)}
						<li>
							...
							>
								...
								<small>v{note.version} • {formatTimestamp(note.updated_at_unix_ms)}</small>
							</button>
						</li>
					{:else}
		...
		<section class="panel editor">
			{#if selectedNote !== null}
				...
				>
					...
					<p class="hint">
						Updated {formatTimestamp(selectedNote.updated_at_unix_ms)} • version {selectedNote.version}
					</p>
				</form>
			{:else}
				<div class="empty">
